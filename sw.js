const CACHE='hamburg-park-v1.7.3';
const SHELL=['./','./index.html','./manifest.json','./icon.svg','./localization-fix.js','./onboarding-ui.js','./live-location-fix.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(isDocument){
    event.respondWith(fetch(event.request).then(async response=>{
      ...
      if(!html.includes('localization-fix.js'))html=html.replace('</body>','<script src="./localization-fix.js?v=1.7.3"></script></body>');
      if(!html.includes('onboarding-ui.js'))html=html.replace('</body>','<script src="./onboarding-ui.js?v=1.7.3"></script></body>');
      if(!html.includes('live-location-fix.js'))html=html.replace('</body>','<script src="./live-location-fix.js?v=1.7.3"></script></body>');
