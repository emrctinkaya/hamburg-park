const CACHE='hamburg-park-v1.7.6';
...
self.addEventListener('fetch',event=>{
  ...
  if(isDocument){
    event.respondWith(fetch(event.request).then(async response=>{
      ...
      if(!html.includes('localization-fix.js'))html=html.replace('</body>','<script src="./localization-fix.js?v=1.7.6"></script></body>');
      if(!html.includes('onboarding-ui.js'))html=html.replace('</body>','<script src="./onboarding-ui.js?v=1.7.6"></script></body>');
      if(!html.includes('live-location-fix.js'))html=html.replace('</body>','<script src="./live-location-fix.js?v=1.7.6"></script></body>');
      if(!html.includes('flag-language-picker.js'))html=html.replace('</body>','<script src="./flag-language-picker.js?v=1.7.6"></script></body>');
      if(!html.includes('ux-polish.js'))html=html.replace('</body>','<script src="./ux-polish.js?v=1.7.6"></script></body>');
