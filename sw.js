const CACHE='hamburg-park-v1.7.2';
...
self.addEventListener('fetch',event=>{
  ...
  if(isDocument){
    event.respondWith(fetch(event.request).then(async response=>{
      ...
      if(!html.includes('localization-fix.js'))html=html.replace('</body>','<script src="./localization-fix.js?v=1.7.2"></script></body>');
      if(!html.includes('onboarding-ui.js'))html=html.replace('</body>','<script src="./onboarding-ui.js?v=1.7.2"></script></body>');
